...
export function initPaintShopDigitalTwin() {
  window.addEventListener('error', function (e) {
    ...
    if (typeof THREE === 'undefined') {
      ...
      const BELT_SPEED = 24, MIN_GAP = 24;
      ...
      const dynamicTypes = {};
      const supportedShapes = ['box', 'sphere', 'cylinder', 'cone', 'torus'];
      const usedComponentShapes = new Set(['box', 'cylinder', 'torus']);
      const usedComponentIds = new Set(['A', 'B', 'C']);
        ...
        .then(data => {
          ...
          data.componentTypes.forEach(type => {
           ...
           usedComponentShapes.add(type.shape);
            if (!['A', 'B', 'C'].includes(type.component_id)) {
            ...
            if (sel && !sel.querySelector(`option[value="${type.component_id}"]`)) {
              const opt = document.createElement('option');
              opt.value = type.component_id;
              opt.textContent = `Component ${type.component_id} (${type.component_type})`;
              sel.appendChild(opt);
            }
          });
        })
      ...
      function makeCompMesh(id, type) {
        ...
        if (type === 'A' || !type) {
          ...
          partGrp.add(part);
          // two punched holes near the top of the bracket, like the reference part
          ...
          const cylGeo = new THREE.CylinderGeometry(4.5, 4.5, PART_LEN, 16);
          const part = new THREE.Mesh(cylGeo, partMat);
          part.position.y = -HOOK_LEN - PART_LEN / 2;
          partGrp.add(part);
        } else if (type === 'C') {
          const ringGeo = new THREE.TorusGeometry(12, 3.5, 16, 32);
          const part = new THREE.Mesh(ringGeo, partMat);
          part.position.y = -HOOK_LEN - 18;
          partGrp.add(part);
        } else if (dynamicTypes[type]) {
        ...
        const label = makeLabelTiny('C' + id);
      ...
      function makeLabelTiny(text) {
        ...
        ctx.fillStyle = '#ffd27a'; ctx.font = '700 18px JetBrains Mono, monospace'; ctx.textAlign = 'center';
        ctx.fillText(text, w / 2, h / 2 + 6);
        const tex = new THREE.CanvasTexture(cnv);
        const mat = new THREE.SpriteMaterial({ map: tex, depthTest: false, transparent: true });
        const sp = new THREE.Sprite(mat);
        sp.scale.set(20, 20 * h / w, 1);
        sp.renderOrder = 11;
        return sp;
      }
  ...
  `;
      ...
      function spawn(forcedType = null, forcedPos = 0) {
        ...
        logEvent(`C${String(c.id).padStart(3, '0')} [Component ${type}] entered line`, 'spawn');
      }
      ...
      document.getElementById('btnNewType').addEventListener('click', () => {
        ...
        let typeId;
        do {
          typeId = String.fromCharCode(nextTypeChar++);
        } while (usedComponentIds.has(typeId));
